...
for i in range(1):
	...
	dir_choose = r"Z:\x299\JS寄生虫.720p.1080p.HD中字\6v电影APP 有他不迷路\appsiteweb\PIDEO_ROOT\newRoot"
	...
	for dir_current, list_sub_dirs, list_sub_files in os.walk(dir_choose):
		...
		for i in list_sub_files:
			if ".bc!" in i:
				os.remove(os.path.join(dir_current,i))
		...
		for jav_file in list_jav_files:
			try:
				...
				if os.path.exists(path_json):
					...
					status, genres_bus, javBusHtml = scrape_from_bus(jav_file, jav_model, handler.url_bus,
					                                                 handler.proxy_bus)
					if not javBusHtml:
						javBusHtml=''
					# print('javBusHtml----\n',javBusHtml)
					...
					if '乳首' in jav_model.Title or '乳首' in jav_model.TitleZh or '乳头' in jav_model.TitleZh:
						genres.append('乳首')
					...
					lllist = ["\\", "/", "|", "<", ">", "?", ":", "！", "”", "*", "…"]
					titlezh = str(jav_model.TitleZh)[0:30]
					for ii in lllist:
						titlezh = titlezh.replace(ii, '_')
					jav_model.TitleZh = titlezh
				...
				try:
					handler.donwload_illustration(jav_file=jav_file,javBusHtml=javBusHtml)
				except:
					pass
