...
import re
import requests
from .actorImgGet import get_actor_img
import os,sys
...
def scrape_from_bus(jav_file, jav_model, url_bus, proxy):
	...
	if html_jav_bus:
		...
		print('html_jav_bus',len(html_jav_bus))
		try:
			getImg = get_actor_img()
			getImg.main(hhtml=html_jav_bus, url=url_bus)
	...
	return status, genres,html_jav_bus

if __name__ == '__main__':
	url = r"https://javdb33.com/search?q=DASD-965&f=all"

	rqs = requests.get(url, timeout=(6, 7), headers={'Cookie': 'existmag=all'})
	# a = get_bus_html(url,None)
	print(rqs)