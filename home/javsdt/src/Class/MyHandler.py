...
class Handler(object):
			...
			if not jav_model.Actors:
				...
				for each_actor in jav_model.Actors:
					...
					if os.path.exists(f'{path_exist_actor}.jpg'):
						...
						pic_type = '.png'
					elif os.path.exists(f'{path_exist_actor}.gif'):
						pic_type = '.gif'
					elif os.path.exists(f'{path_exist_actor}.JPG'):
						pic_type = '.JPG'
					elif os.path.exists(f'{path_exist_actor}.PNG'):
						pic_type = '.PNG'
					elif os.path.exists(f'{path_exist_actor}.GIF'):
						...
						config_actor.read('actors_for_kodi.ini', encoding='utf-8-sig')
						try:
	...
	def donwload_illustration(self, jav_file, javBusHtml):
		if not javBusHtml:
			return None
		import json
		import pprint
		...
		soup = BeautifulSoup(javBusHtml, 'lxml')
		illustration = soup.find_all(class_='sample-box')
		illustration_OriginalUrl = [urlparse.urljoin(self.url_bus, x.get('href')) for x in illustration]
		for num, imgUrl in enumerate(illustration_OriginalUrl):
			imgName = jav_file.Car + '_img_{}-fanart.JPG'.format(str(num))
			imgPath = os.path.join(jav_file.Dir, imgName)
			if os.path.exists(imgPath):
				continue
			try:
				print(imgUrl)
				imgBrow = requests.get(imgUrl, timeout=(10, 10))
