...
from wechatsogou.base import WechatSogouBase
...
class WechatSogouBasic(WechatSogouBase):
            ...
            if hasattr(self, '_ocr'):
                ...
                if not result.has_key('Result'):
                    ...
                    cookie_jar.set_cookie(
                        cookielib.Cookie(version=0, name='SNUID', value=remsg['id'], port=None, port_specified=False,
    ...
    def _ocr_for_get_gzh_article_by_url_text(self, url):
        ...
        if hasattr(self, '_ocr'):
            ...
            while (max_count < 3):
                ...
                # print coder
                logger.debug('vcode appear, using _ocr_for_get_gzh_article_by_url_text')
                result = self._ocr.create(coder.content,2040,timeout=60)
                print result
                if not result.has_key('Result'):
                    print(u"若快识别失败，1秒后更换验证码再次尝试，尝试次数：%d" % (max_count))
                    time.sleep(1)
    ...
    def _search_gzh_text(self, name, page=1):
        ...
        try:
            print(ipPool())
            text = self._get(request_url,proxies=ipPool())
            ...
            try:
                ...
                text = self._get(request_url, 'get',
                                 referer='http://weixin.sogou.com/antispider/?from=%2f' + quote(
                                     self._vcode_url.replace('http://', '')),proxies=ipPool())
            except WechatSogouVcodeException:
                text = ""
    ...
    def _get_gzh_article_by_url_text(self, url):
        ...
        text = self._get(url, 'get', host='mp.weixin.qq.com',proxies=ipPool())
    ...
    def _get_gzh_article_text(self, url):
        ...
        Returns:
            text: 文章文本
        """
        return self._get(url, 'get', host='mp.weixin.qq.com',proxies=ipPool())
    ...
    def _deal_related(self, url, title):
        ...
        related_text = self._get(related_req_url, 'get', host='mp.weixin.qq.com', referer=url,proxies=ipPool())
