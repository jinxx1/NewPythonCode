...
a = os.system('python2 auto_add_mp.py')
#数据库实例
mysql = mysql('mp_info')
#循环获取数据库中所有公众号
...
for item in mp_list:
    try:
        ...
        if u'链接已过期' in wz_list:
            print(u'链接过期',item['wx_hao'])
            wechat_info = wechats.get_gzh_info(item['wx_hao'])

            print(u'wechat_info：-----------',wechat_info)
            # if not wechat_info.has_key('url'):
            if not wechat_info:
                continue
            print('guo qi chong xin huo qu success')
            wz_url = wechat_info['url'];
            wz_list = wechats.get_gzh_message(url=wz_url)
        ...
        for wz_item in wz_list :
            ...
            if wz_item['type'] == '49':
                ...
                try:
                    wenzhanP = wechats.deal_article_content(url=wz_item['content_url'])
                except AttributeError:
                    wenzhanP = u'本文无正文，仅一张图片或转链接到其他公众号上。'
                mysql.table('wenzhang_info').add({'title':wz_item['title'],
                                                ...
                                                'like_count':0,
                                                'read_count':0,
                                                'content':wenzhanP.lstrip().rstrip().replace('imgsrc','img src'),
                                                'comment_count':0})
