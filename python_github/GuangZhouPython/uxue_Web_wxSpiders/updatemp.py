    ...
    try:
        ...
        for wz_item in wz_list :
            ...
            if wz_item['type'] == '49':
                ...
                import random
                artcleNum = str(random.randint(100000000000,999999999999))
                index_html_path = wechats.down_html(wz_item['content_url'],wz_item['title'],item['wx_hao'],artcleNum)

                # ret_path：文章indexl地址
                # artcleNum：该文章生成的编号
                # wordT：文章全文
                # coverName：封面地址
                # imgList：文章所有图片地址List

                if not index_html_path:
                    continue

                mysql.table('wenzhang_info').add(
                    {'title':wz_item['title'],
                                                ...
                                                'like_count':0,
                                                'read_count':0,
                                                'artcleNum':index_html_path['artcleNum'],
                                                'comment_count':0,
                                                'zhuanyi':0,
                     'shejiMD':0,
                ...
                # import pprint
                # pprint.pprint(index_html_path)
                mysql.table('local_index_list').add(
                    {                           'ret_path':index_html_path['ret_path'],
                                                'coverName':index_html_path['coverName'],
                                                # 'imgList':index_html_path['imgList'],
                                                'artcleNum':index_html_path['artcleNum'],
                                                'wordT':index_html_path['wordT'].decode('utf-8'),
                                                'zhuanyi': 0,
                                                'intomysqlTime':datetime.datetime.now()
                    })
