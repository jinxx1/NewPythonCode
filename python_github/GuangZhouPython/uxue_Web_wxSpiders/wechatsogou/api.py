...
class WechatSogouApi(WechatSogouBasic):
    ...
    def deal_article_local_content(self, **kwargs):
        """获取文章内容
    ...
    def deal_article_jx_content(self, text):
        """获取文章内容
    ...
    #下载文章到本地
    def down_html(self, url,dir_name,wxhao,artcleName):
        try:
            #获取biz
            ...
            # dir = 'WeiXinGZH/' + wxhao + '/' + ctime + '/' + dir_name + '/'
            artcleNum = ctime + '_' + artcleName
            dir = 'WeiXinGZH/' + wxhao + '/' + ctime + '/' + artcleNum + '/'
            ...
            for link in soup.find_all('img') :
                try:
                    ...
                    if None != link.get('data-src') :
                        ...
                        file_path = dir + file_name #所有图片本地下载地址
            ...
            with open("%sindex.html" % (dir), "wb") as code :
                code.write(unicode("\xEF\xBB\xBF", "utf-8"))
                wordT = self.deal_article_jx_content(html).replace('\n','').strip()
                code.write(wordT)
