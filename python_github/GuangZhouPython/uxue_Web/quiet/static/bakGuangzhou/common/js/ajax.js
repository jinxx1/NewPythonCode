...
/**
 *对象(提交按钮)调用ajax ,
 * 1.自动判断是否为是否为连续点击,连点无效
 * 2.自动加tipsMsg("保存中…");
 * 3.自动把按钮透明度降到0.5,css("opacity", "0.5"),完成后改为1
 * 4.成功或失败:如果外面只是弹出一个提示框,可以不用写
 * 使用方法
 * $("#issueBtn").on("click", function () {
        var $this = $(this);
        $this.ajax2("../../api/wxMenuOrIssue", "get", true, null);
   })
 */
$.prototype.ajax2 = function (url, type, async, param, successFn, errorFn) {
    var $this = $(this);
    if ($this.css("opacity") == 0.5) {
        return false;
    }
    $this.css("opacity", "0.5");
    //tipsMsg((type=='get'?'查询中...':type=='post'?'提交中...':type=='delete'?'删除中...':type=='put'?'修改中...':'处理中...'));
    $.ajax({
        url: url, //请求地址
        type: type,
        async: async,
        ...
        success: function (data) {
            if (data.code == 0) {
                if (successFn != null) {
                    ...
                    console.log("ajax 成功！,未发现成功回调函数,自动弹出提示框:操作成功");
                    if (data.msg != null && data.msg != '' && data.msg != 'success') {
                        tipsMsg(data.msg);
                    } else {
                        tipsMsg("操作成功");
                    }
                ...
                var link = location.href;
                if (data.msg.indexOf("?") != -1) {
                    link = data.msg + '&src=' + location.href;
                } else {
                    link = data.msg + '?src=' + location.href;
                }
                location.href = link;
            } else if (data.code == 800) {//未知错误
