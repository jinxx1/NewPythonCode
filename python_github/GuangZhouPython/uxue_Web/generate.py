...
class Generate(object):
    ...
    def __init__(self):
        ...
        self._post_Bak_folder = POST_BAK_PATH
        self._page_Bak_folder = PAGE_BAK_PATH
        self._posts = []
        self._pages = []
        self._tags = []
        self._categories = []
    ...
    def update_post_data(self, file, data):
        ...
        XXhtml = [fn['filename'] for fn in self._posts]
        if generate_file not in XXhtml:
    ...
    def markdown_to_html(self, file, file_type):
        ...
        with codecs.open(file, mode="r", encoding="utf-8", errors='ignore') as f:
            ...
            content = md.convert(body.encode('utf-8').decode('utf-8-sig'))
            # pprint.pprint(md.Meta)
            meta = md.Meta if hasattr(md, 'Meta') else {}

            if file_type == 'post':
                data = self.parse_meta(file, meta)
                imgTemp = data['authorImg'].split('static')
                data['authorImg'] = ROOT + 'static' + imgTemp[1]
                self.update_post_data(file, data)



                contentmySQL = get_mysql_Content(data['url'])
                ...
                for t, c in zip(tags, counts):
                    ...
                    tagCloudList.append(dict)
                    nn+=1

                template = self.env.get_template('post.html')
                html = template.render(
                    ROOTWEB = ROOT,
                    tagCloudList = tagCloudList,
                    article=content,
                    articlesql = contentmySQL,
                    data=data,
    ...
    def render_index_html(self):
        ...
        index_root = self.save_html('index.html', html, 'page')
        return index_root
    ...
    def render_tag_html(self):
        ...
        tag_root = self.save_html('tags.html', html, 'page')
        return tag_root
    ...
    def render_cate_html(self):
        ...
        cate_root = self.save_html('categories.html', html, 'page')
        return cate_root
        ...
        for p in self._posts:
            for t in tag:
                if t in p.get('tag'):
                    ...
                    filename = t + '.html'
                    tag_posts_root = self.save_html(filename, html, 'page', extra_path='tag')
                tag_posts = []
        return tag_posts_root
    ...
    def render_cate_posts(self, category):
        ...
        cate_posts_root = self.save_html(filename, html, 'page', extra_path='category')
        return cate_posts_root
    ...
    def parse_meta(self, file, meta):
        ...
        articlePreviewImg = meta.get('articlepreviewimg')[0]
        authorImg = meta.get('authorimg')[0]
        ...
        data = {
            ...
            'url': url,
            'id': id,
            'articlePreviewImg':articlePreviewImg,
            'authorImg':authorImg
        }
        return data
    ...
    def generate_post(self):
        ...
        for file in self.load_folder(self._post_folder):
            ...
            # shutil.move(file, self._post_Bak_folder)
    ...
    def generate_page(self):
        ...
        for file in self.load_folder(self._page_folder):
            ...
            # shutil.move(file, self._page_Bak_folder)
