...
def mysql_query(sql):
    ...
    if True:
        ...
        try:
            ...
            print(e)
        return objects
def actual_process(source,mset,tempid):
    ...
    if tempid != None:
        start_id = tempid
    sql = "select a.page_title, b.content, a.purchase_date, a.site, a.guild_id, a.business_type, a.minor_business_type,a.id from ztbInfo as a left join ztbInfoContent as b on a.id = b.info_id where a.id > %s" % (int(start_id))
    if source != "*":
        sql += " and a.purchase_source = %s LIMIT 20" % source
    print(sql)
    objs = mysql_query(sql)
    ret = []
    sknum = 0
    ...
    seqmode = False
    attachmode=False
    outputdir = '/media/terry/4f8380e6-1b3f-4715-97d7-e840b14a8443/outputTest/'
    targetdir = '/media/terry/4f8380e6-1b3f-4715-97d7-e840b14a8443/targetTest/'
    rpath = '/media/terry/4f8380e6-1b3f-4715-97d7-e840b14a8443/fileTest/'
    ...
    for obj in objs:
        ...
        data_back['actual_id'] = str(obj.infoId)
        ...
        if os.path.isfile(filename) and not forcemode:
            ...
            while len(objs) > 0:
                ...
                prjutils.dumpfiles([v], mset, outputdir)
...
if __name__ == "__main__":
    ...
    if os.path.isfile(outputdir + str2 + '.csv'):
        print("file exists:%s" % (outputdir + str2 + '.csv'))
    else:
        prjutils.dumphead(outputdir, str2)
    while True:
        actual_process(int(str1),str2,str3)
        str3 = None
        time.sleep(5)
