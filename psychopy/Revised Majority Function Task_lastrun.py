...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='D:\\APP\\PsychoPy3\\prac_psy\\Revised Majority Function Task\\Revised Majority Function Task_lastrun.py',
...
image_instr = visual.ImageStim(
    win=win,
    name='image_instr', 
    image='image/instr.png', mask=None,
...
mask1 = visual.ImageStim(
    win=win,
    name='mask1', 
    image='image/mask.png', mask=None,
...
while continueRoutine:
    ...
    if text_welcome.status == STARTED and t >= frameRemains:
    ...
    # *text_MFT* updates
    if t >= 1 and text_MFT.status == NOT_STARTED:
        # keep track of start time/frame for later
        text_MFT.tStart = t  # not accounting for scr refresh
    ...
    if t >= 1 and text_indicate.status == NOT_STARTED:
        # keep track of start time/frame for later
        text_indicate.tStart = t  # not accounting for scr refresh
    ...
    # *image_instr* updates
    if t >= 1 and image_instr.status == NOT_STARTED:
        # keep track of start time/frame for later
        image_instr.tStart = t  # not accounting for scr refresh
    ...
    # *key_welcome* updates
    if t >= 1.5 and key_welcome.status == NOT_STARTED:
        # keep track of start time/frame for later
    ...
    # *text_target* updates
    if t >= 1 and text_target.status == NOT_STARTED:
        # keep track of start time/frame for later
        text_target.tStart = t  # not accounting for scr refresh
    ...
    # *text_press* updates
    if t >= 1 and text_press.status == NOT_STARTED:
        # keep track of start time/frame for later
        text_press.tStart = t  # not accounting for scr refresh
    ...
    if t >= 1 and text_continue.status == NOT_STARTED:
        # keep track of start time/frame for later
...
for thisMFTR in MFTR:
    ...
    for thisBlock in blocks:
        ...
        while continueRoutine:
            ...
            # *fix_prep* updates
            if t >= 0 and fix_prep.status == NOT_STARTED:
                # keep track of start time/frame for later
                fix_prep.tStart = t  # not accounting for scr refresh
            ...
            frameRemains = 0 + prepTime- win.monitorFramePeriod * 0.75  # most of one frame period left
            if fix_prep.status == STARTED and t >= frameRemains:
                # keep track of stop time/frame for later
        ...
        for thisTrial in trials:
            ...
            while continueRoutine:
                ...
                # *fix_start* updates
                if t >= 0.0 and fix_start.status == NOT_STARTED:
                    # keep track of start time/frame for later
                    fix_start.tStart = t  # not accounting for scr refresh
                ...
                if fix_start.status == STARTED and t >= frameRemains:
                    # keep track of stop time/frame for later
                ...
                # *mask1* updates
                if t >= ET+0.5 and mask1.status == NOT_STARTED:
                    # keep track of start time/frame for later
                    mask1.tStart = t  # not accounting for scr refresh
                ...
                frameRemains = ET+0.5 + 0.5- win.monitorFramePeriod * 0.75  # most of one frame period left
                if mask1.status == STARTED and t >= frameRemains:
                    # keep track of stop time/frame for later
                ...
                # *mask2* updates
                if t >= ET+0.5 and mask2.status == NOT_STARTED:
                    # keep track of start time/frame for later
                    mask2.tStart = t  # not accounting for scr refresh
                ...
                frameRemains = ET+0.5 + 0.5- win.monitorFramePeriod * 0.75  # most of one frame period left
                if mask2.status == STARTED and t >= frameRemains:
                    # keep track of stop time/frame for later
                ...
                # *mask3* updates
                if t >= ET+0.5 and mask3.status == NOT_STARTED:
                    # keep track of start time/frame for later
                    mask3.tStart = t  # not accounting for scr refresh
                ...
                frameRemains = ET+0.5 + 0.5- win.monitorFramePeriod * 0.75  # most of one frame period left
                if mask3.status == STARTED and t >= frameRemains:
                    # keep track of stop time/frame for later
                ...
                # *mask4* updates
                if t >= ET+0.5 and mask4.status == NOT_STARTED:
                    # keep track of start time/frame for later
                    mask4.tStart = t  # not accounting for scr refresh
                ...
                frameRemains = ET+0.5 + 0.5- win.monitorFramePeriod * 0.75  # most of one frame period left
                if mask4.status == STARTED and t >= frameRemains:
                    # keep track of stop time/frame for later
                ...
                # *mask5* updates
                if t >= ET+0.5 and mask5.status == NOT_STARTED:
                    # keep track of start time/frame for later
                    mask5.tStart = t  # not accounting for scr refresh
                ...
                frameRemains = ET+0.5 + 0.5- win.monitorFramePeriod * 0.75  # most of one frame period left
                if mask5.status == STARTED and t >= frameRemains:
                    # keep track of stop time/frame for later
                ...
                # *mask6* updates
                if t >= ET+0.5 and mask6.status == NOT_STARTED:
                    # keep track of start time/frame for later
                    mask6.tStart = t  # not accounting for scr refresh
                ...
                frameRemains = ET+0.5 + 0.5- win.monitorFramePeriod * 0.75  # most of one frame period left
                if mask6.status == STARTED and t >= frameRemains:
                    # keep track of stop time/frame for later
                ...
                # *mask7* updates
                if t >= ET+0.5 and mask7.status == NOT_STARTED:
                    # keep track of start time/frame for later
                    mask7.tStart = t  # not accounting for scr refresh
                ...
                frameRemains = ET+0.5 + 0.5- win.monitorFramePeriod * 0.75  # most of one frame period left
                if mask7.status == STARTED and t >= frameRemains:
                    # keep track of stop time/frame for later
                ...
                # *mask8* updates
                if t >= ET+0.5 and mask8.status == NOT_STARTED:
                    # keep track of start time/frame for later
                    mask8.tStart = t  # not accounting for scr refresh
                ...
                frameRemains = ET+0.5 + 0.5- win.monitorFramePeriod * 0.75  # most of one frame period left
                if mask8.status == STARTED and t >= frameRemains:
                    # keep track of stop time/frame for later
                ...
                if t >= 0.5 and image1.status == NOT_STARTED:
                    # keep track of start time/frame for later
                    image1.tStart = t  # not accounting for scr refresh
                ...
                frameRemains = 0.5 + ET- win.monitorFramePeriod * 0.75  # most of one frame period left
                ...
                # *image3* updates
                if t >= 0.5 and image3.status == NOT_STARTED:
                    # keep track of start time/frame for later
                ...
                # *image4* updates
                if t >= 0.5 and image4.status == NOT_STARTED:
                    # keep track of start time/frame for later
                    image4.tStart = t  # not accounting for scr refresh
                ...
                frameRemains = 0.5 + ET- win.monitorFramePeriod * 0.75  # most of one frame period left
                if image4.status == STARTED and t >= frameRemains:
                    # keep track of stop time/frame for later
                ...
                # *image5* updates
                if t >= 0.5 and image5.status == NOT_STARTED:
                    # keep track of start time/frame for later
                    image5.tStart = t  # not accounting for scr refresh
                ...
                frameRemains = 0.5 + ET- win.monitorFramePeriod * 0.75  # most of one frame period left
                if image5.status == STARTED and t >= frameRemains:
                    # keep track of stop time/frame for later
            ...
            while continueRoutine and routineTimer.getTime() > 0:
                ...
                # *fix_end* updates
                if t >= 0.75 and fix_end.status == NOT_STARTED:
                    # keep track of start time/frame for later
                ...
                frameRemains = 0.75 + 1.25- win.monitorFramePeriod * 0.75  # most of one frame period left
        ...
        while continueRoutine:
            ...
            # *key_rest* updates
            if t >= 0 and key_rest.status == NOT_STARTED:
                # keep track of start time/frame for later
                key_rest.tStart = t  # not accounting for scr refresh
                ...
                # keyboard checking is just starting
                key_rest.clearEvents(eventType='keyboard')
            if key_rest.status == STARTED and bool(rest):
                # keep track of stop time/frame for later
            ...
            # *text_rest* updates
            if t >= 0 and text_rest.status == NOT_STARTED:
                # keep track of start time/frame for later
                text_rest.tStart = t  # not accounting for scr refresh
            ...
            if text_rest.status == STARTED and bool(rest):
                # keep track of stop time/frame for later
...
while continueRoutine:
    ...
    if t >= 0.0 and key_end.status == NOT_STARTED:
        # keep track of start time/frame for later
        key_end.tStart = t  # not accounting for scr refresh
