...
class BloomFilter(object):
	def __init__(self, key, bit=BLOOMFILTER_BIT, hash_number=BLOOMFILTER_HASH_NUMBER):
		...
		self.rediServer = redis.StrictRedis(host='localhost', port=6379, db=1, decode_responses=True, encoding='utf-8')
		self.key = key
	...
	def exists(self, value):
		...
		for map in self.maps:
			...
			exist = exist & self.rediServer.getbit(self.key, offset)
		return exist == 1
	...
	def insert(self, value):
		...
		for f in self.maps:
			offset = f.hash(value)
			self.rediServer.setbit(self.key, offset, 1)


bl = BloomFilter("uxue:url")
bh = BloomFilter("uxuehub:huburl")
