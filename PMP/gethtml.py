import pprint

import sqlalchemy
import re
import os, sys

import json
from bs4 import BeautifulSoup

root = os.getcwd()

MYSQLINFO = {
    "HOST": "localhost",
    "DBNAME": "pmp",
    "USER": "root",
    "PASSWORD": "040304",
    "PORT": 3306
}

conStr = 'mysql+pymysql://{USER}:{PASSWORD}@{HOST}:{PORT}/{DBNAME}?charset=utf8mb4'.format(USER=MYSQLINFO['USER'],
                                                                                           PASSWORD=MYSQLINFO[
                                                                                               'PASSWORD'],
                                                                                           HOST=MYSQLINFO['HOST'],
                                                                                           PORT=MYSQLINFO['PORT'],
                                                                                           DBNAME=MYSQLINFO[
                                                                                               'DBNAME'])

mysqlcon = sqlalchemy.create_engine(conStr)

charpter = [{'chapter_id': 178, 'exam_title': 'PMP第8章 项目质量管理', 'section_id': 231,
             'quest_id': [10617, 10618, 10619, 10620, 10621, 10622, 10623, 10624, 10625, 10626, 10627, 10628, 10629,
                          10630, 10631, 10632, 10633, 10634, 10635, 10636, 10637, 10638, 10639, 10640, 10641, 10642,
                          10643, 10644, 10645, 10646, 10647, 10648, 10649, 10650, 10651, 10652, 10653, 10654, 10655,
                          10656, 10657, 10658, 10659, 10660, 10661, 10662, 10663, 10664, 10665, 10666, 10667, 10668,
                          10669, 10670, 10671, 10672, 10673, 10674, 10675, 10676, 10977, 10978, 10979]},
            {'chapter_id': 179, 'exam_title': 'PMP第1-3 章习题', 'section_id': 232,
             'quest_id': [10293, 10294, 10295, 10296, 10297, 10298, 10299, 10300, 10301, 10302, 10303, 10304, 10305,
                          10306, 10307, 10308, 10309, 10310, 10311, 10312, 10313, 10314, 10315, 10316, 10317, 10318,
                          10319, 10320, 10321, 10322, 10323, 10324, 10325, 10326, 10327, 10328, 10329, 10330, 10331,
                          10332, 10333, 10334, 10335, 10336, 10337, 10338, 10339, 10340, 10341, 10342, 10343, 10344,
                          10345, 10346, 10347, 10348, 10349, 10350, 10351, 10352]},
            {'chapter_id': 180, 'exam_title': 'PMP第4章 项目整合管理', 'section_id': 233,
             'quest_id': [10353, 10354, 10355, 10356, 10357, 10358, 10359, 10360, 10361, 10362, 10363, 10364, 10365,
                          10366, 10367, 10368, 10369, 10370, 10371, 10372, 10373, 10374, 10375, 10376, 10377, 10378,
                          10379, 10380, 10381, 10382, 10383, 10384, 10385, 10386, 10387, 10388, 10389, 10390, 10391,
                          10392, 10393, 10394, 10395, 10396, 10397, 10398, 10399, 10400, 10401, 10402, 10403, 10404,
                          10405, 10406, 10407, 10408, 10409, 10410, 10411, 10412]},
            {'chapter_id': 181, 'exam_title': 'PMP第5章  项目范围管理', 'section_id': 234,
             'quest_id': [10413, 10414, 10415, 10416, 10417, 10418, 10419, 10420, 10421, 10422, 10423, 10424, 10425,
                          10426, 10427, 10428, 10429, 10430, 10431, 10432, 10433, 10434, 10435, 10436, 10437, 10438,
                          10439, 10440, 10441, 10442, 10443, 10444, 10445, 10446, 10447, 10448, 10449, 10450, 10451,
                          10452, 10453, 10454, 10455, 10456, 10457, 10458, 10459, 10460, 10461, 10462, 10463, 10464,
                          10465, 10466, 10467, 10468, 10469, 10470, 10471, 10472]},
            {'chapter_id': 182, 'exam_title': 'PMP第6章  项目进度管理', 'section_id': 235,
             'quest_id': [10497, 10498, 10499, 10500, 10501, 10502, 10503, 10504, 10505, 10506, 10507, 10508, 10509,
                          10510, 10511, 10512, 10513, 10514, 10515, 10516, 10517, 10518, 10519, 10520, 10521, 10522,
                          10523, 10524, 10525, 10526, 10527, 10528, 10529, 10530, 10531, 10532, 10533, 10534, 10535,
                          10536, 10537, 10538, 10539, 10540, 10541, 10542, 10543, 10544, 10545, 10546, 10547, 10548,
                          10549, 10550, 10551, 10552, 10553, 10554, 10555, 10556]},
            {'chapter_id': 183, 'exam_title': 'PMP第7章  项目成本管理', 'section_id': 236,
             'quest_id': [2481, 10557, 10558, 10559, 10560, 10561, 10562, 10563, 10564, 10565, 10566, 10567, 10568,
                          10569, 10570, 10571, 10572, 10573, 10574, 10575, 10576, 10577, 10578, 10579, 10580, 10581,
                          10582, 10583, 10584, 10585, 10586, 10587, 10588, 10589, 10590, 10591, 10592, 10593, 10594,
                          10595, 10596, 10597, 10598, 10599, 10600, 10601, 10602, 10603, 10604, 10605, 10606, 10607,
                          10608, 10610, 10611, 10612, 10613, 10614, 10615, 10616]},
            {'chapter_id': 184, 'exam_title': 'PMP第9章  项目资源管理', 'section_id': 237,
             'quest_id': [10677, 10678, 10679, 10680, 10681, 10682, 10683, 10684, 10685, 10686, 10687, 10688, 10689,
                          10690, 10691, 10692, 10693, 10694, 10695, 10696, 10697, 10698, 10699, 10700, 10701, 10702,
                          10703, 10704, 10705, 10706, 10707, 10708, 10709, 10710, 10711, 10712, 10713, 10714, 10715,
                          10716, 10717, 10718, 10719, 10720, 10721, 10722, 10723, 10724, 10725, 10726, 10727, 10728,
                          10729, 10730, 10731, 10732, 10733, 10734, 10735, 10736]},
            {'chapter_id': 185, 'exam_title': 'PMP第10章  项目沟通管理', 'section_id': 238,
             'quest_id': [10737, 10738, 10739, 10740, 10741, 10742, 10743, 10744, 10745, 10746, 10747, 10748, 10749,
                          10750, 10751, 10752, 10753, 10754, 10755, 10756, 10757, 10758, 10759, 10760, 10761, 10762,
                          10763, 10764, 10765, 10766, 10767, 10768, 10769, 10770, 10771, 10772, 10773, 10774, 10775,
                          10776, 10777, 10778, 10779, 10780, 10781, 10782, 10783, 10784, 10785, 10786, 10787, 10788,
                          10789, 10790, 10791, 10792, 10793, 10794, 10795, 10796]},
            {'chapter_id': 186, 'exam_title': 'PMP第11章  项目风险管理', 'section_id': 239,
             'quest_id': [10797, 10798, 10799, 10800, 10801, 10802, 10803, 10804, 10805, 10806, 10807, 10808, 10809,
                          10810, 10811, 10812, 10813, 10814, 10815, 10816, 10817, 10818, 10819, 10820, 10821, 10822,
                          10823, 10824, 10825, 10826, 10827, 10828, 10829, 10830, 10831, 10832, 10833, 10834, 10835,
                          10836, 10837, 10838, 10839, 10840, 10841, 10842, 10843, 10844, 10845, 10846, 10847, 10848,
                          10849, 10850, 10851, 10852, 10853, 10854, 10855, 10856]},
            {'chapter_id': 187, 'exam_title': 'PMP第12章  项目采购管理', 'section_id': 240,
             'quest_id': [10857, 10858, 10859, 10860, 10861, 10862, 10863, 10864, 10865, 10866, 10867, 10868, 10869,
                          10870, 10871, 10872, 10873, 10874, 10875, 10876, 10877, 10878, 10879, 10880, 10881, 10882,
                          10883, 10884, 10885, 10886, 10887, 10888, 10889, 10890, 10891, 10892, 10893, 10894, 10895,
                          10896, 10897, 10898, 10899, 10900, 10901, 10902, 10903, 10904, 10905, 10906, 10907, 10908,
                          10909, 10910, 10911, 10912, 10913, 10914, 10915, 10916]},
            {'chapter_id': 188, 'exam_title': 'PMP第13章  项目相关方管理', 'section_id': 241,
             'quest_id': [10917, 10918, 10919, 10920, 10921, 10922, 10923, 10924, 10925, 10926, 10927, 10928, 10929,
                          10930, 10931, 10932, 10933, 10934, 10935, 10936, 10937, 10938, 10939, 10940, 10941, 10942,
                          10943, 10944, 10945, 10946, 10947, 10948, 10949, 10950, 10951, 10952, 10953, 10954, 10955,
                          10956, 10957, 10958, 10959, 10960, 10961, 10962, 10963, 10964, 10965, 10966, 10967, 10968,
                          10969, 10970, 10971, 10972, 10973, 10974, 10975, 10976]}]
paper = [{'exam_title': '远程PMP全真模拟二', 'exam_id': 50472,
          'quest_id': [11380, 11381, 11382, 11383, 11384, 11385, 11386, 11387, 11388, 11389, 11390, 11391, 11392, 11393,
                       11394, 11395, 11396, 11397, 11398, 11399, 11400, 11401, 11402, 11403, 11404, 11405, 11406, 11407,
                       11408, 11409, 11410, 11411, 11412, 11413, 11414, 11415, 11416, 11417, 11418, 11419, 11420, 11421,
                       11422, 11423, 11424, 11425, 11426, 11427, 11428, 11429, 11430, 11431, 11432, 11433, 11434, 11435,
                       11436, 11437, 11438, 11439, 11440, 11441, 11442, 11443, 11444, 11445, 11446, 11447, 11448, 11449,
                       11450, 11451, 11452, 11453, 11454, 11455, 11456, 11457, 11458, 11459, 11460, 11461, 11462, 11463,
                       11464, 11465, 11466, 11467, 11468, 11469, 11470, 11471, 11472, 11473, 11474, 11475, 11476, 11477,
                       11478, 11479, 11480, 11481, 11482, 11483, 11484, 11485, 11486, 11487, 11488, 11489, 11490, 11491,
                       11492, 11493, 11494, 11495, 11496, 11497, 11498, 11499, 11500, 11501, 11502, 11503, 11504, 11505,
                       11506, 11507, 11508, 11509, 11510, 11511, 11512, 11513, 11514, 11515, 11516, 11517, 11518, 11519,
                       11520, 11521, 11522, 11523, 11524, 11525, 11526, 11527, 11528, 11529, 11530, 11531, 11532, 11533,
                       11534, 11535, 11536, 11537, 11538, 11539, 11540, 11541, 11542, 11543, 11544, 11545, 11546, 11547,
                       11548, 11549, 11550, 11551, 11552, 11553, 11554, 11555, 11556, 11557, 11558, 11559, 11560, 11561,
                       11562, 11563, 11564, 11565, 11566, 11567, 11568, 11569, 11570, 11571, 11572, 11573, 11574, 11575,
                       11576, 11577, 11578, 11579]}, {'exam_title': '远程PMP全真模拟一', 'exam_id': 48938,
                                                      'quest_id': [10980, 10981, 10982, 10983, 10984, 10985, 10986,
                                                                   10987, 10988, 10989, 10990, 10991, 10992, 10993,
                                                                   10994, 10995, 10996, 10997, 10998, 10999, 11000,
                                                                   11001, 11002, 11003, 11004, 11005, 11006, 11007,
                                                                   11008, 11009, 11010, 11011, 11012, 11013, 11014,
                                                                   11015, 11016, 11017, 11018, 11019, 11020, 11021,
                                                                   11022, 11023, 11024, 11025, 11026, 11027, 11028,
                                                                   11029, 11030, 11031, 11032, 11033, 11034, 11035,
                                                                   11036, 11037, 11038, 11039, 11040, 11041, 11042,
                                                                   11043, 11044, 11045, 11046, 11047, 11048, 11049,
                                                                   11050, 11051, 11052, 11053, 11054, 11055, 11056,
                                                                   11057, 11058, 11059, 11060, 11061, 11062, 11063,
                                                                   11064, 11065, 11066, 11067, 11068, 11069, 11070,
                                                                   11071, 11072, 11073, 11074, 11075, 11076, 11077,
                                                                   11078, 11079, 11080, 11081, 11082, 11083, 11084,
                                                                   11085, 11086, 11087, 11088, 11089, 11090, 11091,
                                                                   11092, 11093, 11094, 11095, 11096, 11097, 11098,
                                                                   11099, 11100, 11101, 11102, 11103, 11104, 11105,
                                                                   11106, 11107, 11108, 11109, 11110, 11111, 11112,
                                                                   11113, 11114, 11115, 11116, 11117, 11118, 11119,
                                                                   11120, 11121, 11122, 11123, 11124, 11125, 11126,
                                                                   11127, 11128, 11129, 11130, 11131, 11132, 11133,
                                                                   11134, 11135, 11136, 11137, 11138, 11139, 11140,
                                                                   11141, 11142, 11143, 11144, 11145, 11146, 11147,
                                                                   11148, 11149, 11150, 11151, 11152, 11153, 11154,
                                                                   11155, 11156, 11157, 11158, 11159, 11160, 11161,
                                                                   11162, 11163, 11164, 11165, 11166, 11167, 11168,
                                                                   11169, 11170, 11171, 11172, 11173, 11174, 11175,
                                                                   11176, 11177, 11178, 11179]},
         {'exam_title': '综合练习二--远程', 'exam_id': 51263,
          'quest_id': [11782, 11783, 11784, 11785, 11786, 11787, 11788, 11789, 11790, 11791, 11792, 11793, 11794, 11795,
                       11796, 11797, 11798, 11799, 11800, 11801, 11802, 11803, 11804, 11805, 11806, 11807, 11808, 11809,
                       11810, 11811, 11812, 11813, 11814, 11815, 11816, 11817, 11818, 11819, 11820, 11821, 11822, 11823,
                       11824, 11825, 11826, 11827, 11828, 11829, 11830, 11831, 11832, 11833, 11834, 11835, 11836, 11837,
                       11838, 11839, 11840, 11841, 11842, 11843, 11844, 11845, 11846, 11847, 11848, 11849, 11850, 11851,
                       11852, 11853, 11854, 11855, 11856, 11857, 11858, 11859, 11860, 11861, 11862, 11863, 11864, 11865,
                       11866, 11867, 11868, 11869, 11870, 11871, 11872, 11873, 11874, 11875, 11876, 11877, 11878, 11879,
                       11880, 11881, 11882, 11883, 11884, 11885, 11886, 11887, 11888, 11889, 11890, 11891, 11892, 11893,
                       11894, 11895, 11896, 11897, 11898, 11899, 11900, 11901, 11902, 11903, 11904, 11905, 11906, 11907,
                       11908, 11909, 11910, 11911, 11912, 11913, 11914, 11915, 11916, 11917, 11918, 11919, 11920, 11921,
                       11922, 11923, 11924, 11925, 11926, 11927, 11928, 11929, 11930, 11931, 11932, 11933, 11934, 11935,
                       11936, 11937, 11938, 11939, 11940, 11941, 11942, 11943, 11944, 11945, 11946, 11947, 11948, 11949,
                       11950, 11951, 11952, 11953, 11954, 11955, 11956, 11957, 11958, 11959, 11960, 11961, 11962, 11963,
                       11964, 11965, 11966, 11967, 11968, 11969, 11970, 11971, 11972, 11973, 11974, 11975, 11976, 11977,
                       11978, 11979, 11980, 11981]}, {'exam_title': '综合练习三--远程', 'exam_id': 52298,
                                                      'quest_id': [12182, 12183, 12184, 12185, 12186, 12187, 12188,
                                                                   12189, 12190, 12191, 12192, 12193, 12194, 12195,
                                                                   12196, 12197, 12198, 12199, 12200, 12201, 12202,
                                                                   12203, 12204, 12205, 12206, 12207, 12208, 12209,
                                                                   12210, 12211, 12212, 12213, 12214, 12215, 12216,
                                                                   12217, 12218, 12219, 12220, 12221, 12222, 12223,
                                                                   12224, 12225, 12226, 12227, 12228, 12229, 12230,
                                                                   12231, 12232, 12233, 12234, 12235, 12236, 12237,
                                                                   12238, 12239, 12240, 12241, 12242, 12243, 12244,
                                                                   12245, 12246, 12247, 12248, 12249, 12250, 12251,
                                                                   12252, 12253, 12254, 12255, 12256, 12257, 12258,
                                                                   12259, 12260, 12261, 12262, 12263, 12264, 12265,
                                                                   12266, 12267, 12268, 12269, 12270, 12271, 12272,
                                                                   12273, 12274, 12275, 12276, 12277, 12278, 12279,
                                                                   12280, 12281, 12282, 12283, 12284, 12285, 12286,
                                                                   12287, 12288, 12289, 12290, 12291, 12292, 12293,
                                                                   12294, 12295, 12296, 12297, 12298, 12299, 12300,
                                                                   12301, 12302, 12303, 12304, 12305, 12306, 12307,
                                                                   12308, 12309, 12310, 12311, 12312, 12313, 12314,
                                                                   12315, 12316, 12317, 12318, 12319, 12320, 12321,
                                                                   12322, 12323, 12324, 12325, 12326, 12327, 12328,
                                                                   12329, 12330, 12331, 12332, 12333, 12334, 12335,
                                                                   12336, 12337, 12338, 12339, 12340, 12341, 12342,
                                                                   12343, 12344, 12345, 12346, 12347, 12348, 12349,
                                                                   12350, 12351, 12352, 12353, 12354, 12355, 12356,
                                                                   12357, 12358, 12359, 12360, 12361, 12362, 12363,
                                                                   12364, 12365, 12366, 12367, 12368, 12369, 12370,
                                                                   12371, 12372, 12373, 12374, 12375, 12376, 12377,
                                                                   12378, 12379, 12380, 12381]},
         {'exam_title': '综合练习四--远程', 'exam_id': 78439,
          'quest_id': [13069, 13070, 13071, 13072, 13073, 13074, 13075, 13076, 13077, 13078, 13079, 13080, 13081, 13082,
                       13083, 13084, 13085, 13086, 13087, 13088, 13089, 13090, 13091, 13092, 13093, 13094, 13095, 13096,
                       13097, 13098, 13099, 13100, 13101, 13102, 13103, 13104, 13105, 13106, 13107, 13108, 13109, 13110,
                       13111, 13112, 13113, 13114, 13115, 13116, 13117, 13118, 13119, 13120, 13121, 13122, 13123, 13124,
                       13125, 13126, 13127, 13128, 13129, 13130, 13131, 13132, 13133, 13134, 13135, 13136, 13137, 13138,
                       13139, 13140, 13141, 13142, 13143, 13144, 13145, 13146, 13147, 13148, 13149, 13150, 13151, 13152,
                       13153, 13154, 13155, 13156, 13157, 13158, 13159, 13160, 13161, 13162, 13163, 13164, 13165, 13166,
                       13167, 13168, 13169, 13170, 13171, 13172, 13173, 13174, 13175, 13176, 13177, 13178, 13179, 13180,
                       13181, 13182, 13183, 13184, 13185, 13186, 13187, 13188, 13189, 13190, 13191, 13192, 13193, 13194,
                       13195, 13196, 13197, 13198, 13199, 13200, 13201, 13202, 13203, 13204, 13205, 13206, 13207, 13208,
                       13209, 13210, 13211, 13212, 13213, 13214, 13215, 13216, 13217, 13218, 13219, 13220, 13221, 13222,
                       13223, 13224, 13225, 13226, 13227, 13228, 13229, 13230, 13231, 13232, 13233, 13234, 13235, 13236,
                       13237, 13238, 13239, 13240, 13241, 13242, 13243, 13244, 13245, 13246, 13247, 13248, 13249, 13250,
                       13251, 13252, 13253, 13254, 13255, 13256, 13257, 13258, 13259, 13260, 13261, 13262, 13263, 13264,
                       13265, 13266, 13267, 13268]}, {'exam_title': '远程PMP全真模拟三', 'exam_id': 51590,
                                                      'quest_id': [11982, 11983, 11984, 11985, 11986, 11987, 11988,
                                                                   11989, 11990, 11991, 11992, 11993, 11994, 11995,
                                                                   11996, 11997, 11998, 11999, 12000, 12001, 12002,
                                                                   12003, 12004, 12005, 12006, 12007, 12008, 12009,
                                                                   12010, 12011, 12012, 12013, 12014, 12015, 12016,
                                                                   12017, 12018, 12019, 12020, 12021, 12022, 12023,
                                                                   12024, 12025, 12026, 12027, 12028, 12029, 12030,
                                                                   12031, 12032, 12033, 12034, 12035, 12036, 12037,
                                                                   12038, 12039, 12040, 12041, 12042, 12043, 12044,
                                                                   12045, 12046, 12047, 12048, 12049, 12050, 12051,
                                                                   12052, 12053, 12054, 12055, 12056, 12057, 12058,
                                                                   12059, 12060, 12061, 12062, 12063, 12064, 12065,
                                                                   12066, 12067, 12068, 12069, 12070, 12071, 12072,
                                                                   12073, 12074, 12075, 12076, 12077, 12078, 12079,
                                                                   12080, 12081, 12082, 12083, 12084, 12085, 12086,
                                                                   12087, 12088, 12089, 12090, 12091, 12092, 12093,
                                                                   12094, 12095, 12096, 12097, 12098, 12099, 12100,
                                                                   12101, 12102, 12103, 12104, 12105, 12106, 12107,
                                                                   12108, 12109, 12110, 12111, 12112, 12113, 12114,
                                                                   12115, 12116, 12117, 12118, 12119, 12120, 12121,
                                                                   12122, 12123, 12124, 12125, 12126, 12127, 12128,
                                                                   12129, 12130, 12131, 12132, 12133, 12134, 12135,
                                                                   12136, 12137, 12138, 12139, 12140, 12141, 12142,
                                                                   12143, 12144, 12145, 12146, 12147, 12148, 12149,
                                                                   12150, 12151, 12152, 12153, 12154, 12155, 12156,
                                                                   12157, 12158, 12159, 12160, 12161, 12162, 12163,
                                                                   12164, 12165, 12166, 12167, 12168, 12169, 12170,
                                                                   12171, 12172, 12173, 12174, 12175, 12176, 12177,
                                                                   12178, 12179, 12180, 12181]},
         {'exam_title': '综合练习一--远程', 'exam_id': 49851,
          'quest_id': [11180, 11181, 11182, 11183, 11184, 11185, 11186, 11187, 11188, 11189, 11190, 11191, 11192, 11193,
                       11194, 11195, 11196, 11197, 11198, 11199, 11200, 11201, 11202, 11203, 11204, 11205, 11206, 11207,
                       11208, 11209, 11210, 11211, 11212, 11213, 11214, 11215, 11216, 11217, 11218, 11219, 11220, 11221,
                       11222, 11223, 11224, 11225, 11226, 11227, 11228, 11229, 11230, 11231, 11232, 11233, 11234, 11235,
                       11236, 11237, 11238, 11239, 11240, 11241, 11242, 11243, 11244, 11245, 11246, 11247, 11248, 11249,
                       11250, 11251, 11252, 11253, 11254, 11255, 11256, 11257, 11258, 11259, 11260, 11261, 11262, 11263,
                       11264, 11265, 11266, 11267, 11268, 11269, 11270, 11271, 11272, 11273, 11274, 11275, 11276, 11277,
                       11278, 11279, 11280, 11281, 11282, 11283, 11284, 11285, 11286, 11287, 11288, 11289, 11290, 11291,
                       11292, 11293, 11294, 11295, 11296, 11297, 11298, 11299, 11300, 11301, 11302, 11303, 11304, 11305,
                       11306, 11307, 11308, 11309, 11310, 11311, 11312, 11313, 11314, 11315, 11316, 11317, 11318, 11319,
                       11320, 11321, 11322, 11323, 11324, 11325, 11326, 11327, 11328, 11329, 11330, 11331, 11332, 11333,
                       11334, 11335, 11336, 11337, 11338, 11339, 11340, 11341, 11342, 11343, 11344, 11345, 11346, 11347,
                       11348, 11349, 11350, 11351, 11352, 11353, 11354, 11355, 11356, 11357, 11358, 11359, 11360, 11361,
                       11362, 11363, 11364, 11365, 11366, 11367, 11368, 11369, 11370, 11371, 11372, 11373, 11374, 11375,
                       11376, 11377, 11378, 11379]}]


def htmlget(html1):
    soup = BeautifulSoup(html1, 'lxml')
    try:
        img = soup.find('img')
        img_html = '''<img src="{}">'''.format(img.get('src'))
        html_title = "<p>" + img_html + "</p>" + "<h2>" + filter_tags(html1) + "</h2>"
    except:
        html_title = "<h2>" + filter_tags(html1) + "</h2>"
    return html_title


def get_mysqlinfo(limit=None):
    if not limit:
        exc = '''select exam_title,quest_answers,title,A,B,C,D,quest_analysis,quest_id from exam;'''
    else:
        exc = '''select exam_title,quest_answers,title,A,B,C,D,quest_analysis,quest_id from exam LIMIT {};'''.format(
            limit)
    # exc = '''select exam_title,quest_answers,title,A,B,C,D,quest_analysis,quest_id from exam where id in (2093,2094,2095,1297) limit 4'''
    a = mysqlcon.execute(exc)
    llist = []
    for i in a:
        ddict = {}
        ddict['exam_title'] = i[0]
        ddict['quest_answers'] = i[1]
        ddict['title'] = htmlget(i[2])
        ddict['A'] = i[3]
        ddict['B'] = i[4]
        ddict['C'] = i[5]
        ddict['D'] = i[6]
        ddict['quest_analysis'] = i[7]
        ddict['quest_id'] = i[8]
        llist.append(ddict)
    return llist


def filter_tags(htmlstr):
    # 先过滤CDATA
    re_cdata = re.compile('//<!\[CDATA\[[^>]*//\]\]>', re.I)  # 匹配CDATA
    re_script = re.compile('<\s*script[^>]*>[^<]*<\s*/\s*script\s*>', re.I)  # Script
    re_style = re.compile('<\s*style[^>]*>[^<]*<\s*/\s*style\s*>', re.I)  # style
    re_br = re.compile('<br\s*?/?>')  # 处理换行
    re_h = re.compile('</?\w+[^>]*>')  # HTML标签
    re_comment = re.compile('<!--[^>]*-->')  # HTML注释
    s = re_cdata.sub('', htmlstr)  # 去掉CDATA
    s = re_script.sub('', s)  # 去掉SCRIPT
    s = re_style.sub('', s)  # 去掉style
    s = re_br.sub('\n', s)  # 将br转换为换行
    s = re_h.sub('', s)  # 去掉HTML 标签
    s = re_comment.sub('', s)  # 去掉HTML注释
    # 去掉多余的空行
    blank_line = re.compile('\n+')
    s = blank_line.sub('\n', s)
    s = replaceCharEntity(s)  # 替换实体
    return s.replace('\n', '').replace('"', '').replace('\\', '').replace("   、 ", '、').replace("?n",
                                                                                                '?</h2><h2>').replace(
        "  ", '')


def replaceCharEntity(htmlstr):
    CHAR_ENTITIES = {'nbsp': ' ', '160': ' ',
                     'lt': '<', '60': '<',
                     'gt': '>', '62': '>',
                     'amp': '&', '38': '&',
                     'quot': '"', '34': '"', }

    re_charEntity = re.compile(r'&#?(?P<name>\w+);')
    sz = re_charEntity.search(htmlstr)
    while sz:
        entity = sz.group()  # entity全称，如&gt;
        key = sz.group('name')  # 去除&;后entity,如&gt;为gt
        try:
            htmlstr = re_charEntity.sub(CHAR_ENTITIES[key], htmlstr, 1)
            sz = re_charEntity.search(htmlstr)
        except KeyError:
            # 以空串代替
            htmlstr = re_charEntity.sub('', htmlstr, 1)
            sz = re_charEntity.search(htmlstr)
    return htmlstr


# 每个题目单独生成html
def allhtml_main():
    tempH = '''
	<h1 hidden="hidden">{exam_title}</h1>
	{title}
	<p>A、{A}</p>
	<p>B、{B}</p>
	<p>C、{C}</p>
	<p>D、{D}</p>
	<b title="{analysisall}">鼠标移到此处，查看答案和解析</b>
	<p hidden="hidden">{quest_id}</p>
	'''
    infoList = get_mysqlinfo()
    for i in infoList:
        analysis = '正确答案为：{quest_answers}\n题目解析：{quest_analysis}'.format(quest_answers=i['quest_answers'],
                                                                         quest_analysis=i['quest_analysis'].replace('"',
                                                                                                                    ''))
        html_temp = tempH.format(exam_title=i['exam_title'],
                                 title=i['title'],
                                 A=i['A'].replace('A', '', 1),
                                 B=i['B'].replace('B', '', 1),
                                 C=i['C'].replace('C', '', 1),
                                 D=i['D'].replace('D', '', 1),
                                 quest_id=i['quest_id'],
                                 analysisall=analysis)
        soup = BeautifulSoup(html_temp, 'lxml')

        statusPsth = os.path.join('status', str(i['quest_id']) + '.html')
        namePath = os.path.join(root, statusPsth)
        with open(namePath, 'w', encoding='utf-8') as f:
            # f.write(soup.prettify())
            f.write(html_temp)


# 每个章节和模拟题，成套做出html
def chart_paper_main():
    for num, pa in enumerate(charpter):
        exam_path = os.path.join('exam', pa['exam_title'] + '.html')
        exam_filePathName = os.path.join(root, exam_path)
        html_all = '<h1>{}</h1><hr />'.format(pa['exam_title'])
        for i in pa['quest_id']:
            status_path = os.path.join(root, 'status')
            status_fileName = os.path.join(status_path, str(i) + '.html')
            with open(status_fileName, 'r', encoding='utf-8') as f:
                status_html = f.read()
            html_all += status_html + "<hr />"
        final_html = BeautifulSoup(html_all, 'lxml')
        with open(exam_filePathName, 'w', encoding='utf-8') as file:
            file.write(final_html.prettify())
            file.flush()
            file.close()


def html_get():
    allinfo = get_mysqlinfo()
    mark = 0
    tempH = '''
				<p>{exam_title}<p>
				{title}
				<p>A、{A}</p>
				<p>B、{B}</p>
				<p>C、{C}</p>
				<p>D、{D}</p>
				<b title="{analysisall}">鼠标移到此处，查看答案和解析</b>
				<p hidden="hidden">{quest_id}</p>
				<hr />
				'''
    keyWord = '净现值'
    temp_final = "<h1>全部 {} 相关题目</h1><hr />".format(keyWord)
    exam_path = os.path.join('exam', keyWord + '.html')
    exam_filePathName = os.path.join(root, exam_path)
    for i in allinfo:
        if i['quest_id'] == 13104:
            continue
        answer = i['quest_answers']
        if '粘合' in i[answer] or '粘合' in i['title']:
            print(i[answer])
            analysis = '正确答案为：{quest_answers}\n题目解析：{quest_analysis}'.format(quest_answers=i['quest_answers'],
                                                                             quest_analysis=i['quest_analysis'].replace(
                                                                                 '"',
                                                                                 ''))
            html_temp = tempH.format(exam_title=i['exam_title'],
                                     title=i['title'],
                                     A=i['A'].replace('A', '', 1),
                                     B=i['B'].replace('B', '', 1),
                                     C=i['C'].replace('C', '', 1),
                                     D=i['D'].replace('D', '', 1),
                                     quest_id=i['quest_id'],
                                     analysisall=analysis)
            temp_final += html_temp
            mark += 1
            print(i)
            print('---------------------', mark)
    # soup = BeautifulSoup(temp_final, 'lxml')
    # with open(exam_filePathName, 'w', encoding='utf-8') as file:
    #     file.write(soup.prettify())
    #     file.flush()
    #     file.close()


def word_get():
    from docx import Document
    from docx.shared import Pt
    from docx.oxml.ns import qn

    document = Document()
    allinfo = get_mysqlinfo(limit=None)
    mark = 0
    keyWord = '德尔非'
    document.add_heading('全部 {} 相关题目'.format(keyWord))

    exam_path = os.path.join('exam', keyWord + '.docx')
    exam_filePathName = os.path.join(root, exam_path)

    for num, i in enumerate(allinfo):
        if i['quest_id'] == 13104:
            continue
        answer = i['quest_answers']

        # if '帕累' in i[answer] or '帕累' in i['title']:
        # if '鱼' in i[answer] or '因果' in i[answer] or '石川' in i[answer] or 'why' in i[answer] or '鱼' in i['title'] or '因果图' in i['title'] or '石川' in i['title'] or 'why' in i['title']:
        # if '控制图' in i[answer] or '控制图' in i['title']:
        # if '总价加' in i[answer] or '成本加' in i[answer] or '工料' in i[answer] or '固定总' in i[answer] or '总价加' in i['title'] or '成本加' in i['title'] or '工料' in i['title'] or '固定总' in i['title']:
        # if '关键路' in i[answer] or '关键路' in i['title'] or 'CPM' in i[answer] or 'CPM' in i['title'] or 'cpm' in i[answer] or 'cpm' in i['title']:
        # if '德尔' in i[answer] or '德尔' in i['title']:
        if 'N' in i[answer] or 'N' in i['title'] or '净' in i[answer] or '净' in i['title']:
            document.add_heading(i['title'].replace('<h2>', '').replace('</h2>', '').replace('</h2><h2>', '\n'), 1)
            corrt = [i['A'], i['B'], i['C'], i['D']]
            corrt_str = '\n'.join(corrt)

            paragraph = document.add_paragraph()
            run = paragraph.add_run(corrt_str)
            run.font.size = Pt(10)
            run.font.name = u'宋体'
            r = run._element
            r.rPr.rFonts.set(qn('w:eastAsia'), u'宋体')

            answer = ['正确答案：{}'.format(i['quest_answers']), '问题解析：{}'.format(i['quest_analysis'].replace('"', ''))]
            answer_str = '\n'.join(answer)

            paragraph = document.add_paragraph()
            run = paragraph.add_run(answer_str)
            run.font.size = Pt(8)
            run.font.name = u'宋体'
            run.italic = True
            r = run._element
            r.rPr.rFonts.set(qn('w:eastAsia'), u'宋体')

            mark += 1
            print('---------------------', mark)
    document.save(exam_filePathName)


if __name__ == '__main__':
    html_get()

    pass
