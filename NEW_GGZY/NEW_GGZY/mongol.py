...
if not inputfile:
    usage()
    print
    "ERROR: Please select an input file of hostnames, one hostname per line"
    sys.exit(1)
...
for host in hostnames:
    ...
    try:
        ...
        print
        "connection to " + host + " has timed out moving on"
        continue
    except socket.error:
        print
        "connection failed, moving on"
        continue
    s.send(MESSAGE % ("/", host))
    ...
    try:
        ...
        print
        "connection to " + host + " has timedout moving on, Possibly not a webserver"
        continue
    except socket.error:
        print
        "RST: Possibly already blocked"
        continue
    ...
    if response.find("200 OK") != -1 or response.find("302 Redirect") != -1 or response.find("401 Unauthorized") != -1:
        ...
        try:
            ...
            print
            "connection to " + host + " has timedout moving on"
            continue
        except socket.error:
            print
            "connection to " + host + " has timedout moving on"
        ...
        try:
            ...
            print
            "Appears not to be blocking"

    else:
        print
        "Bad response code from " + host
        # print response
        continue
    s.close()
